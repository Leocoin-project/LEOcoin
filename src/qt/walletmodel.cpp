...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedLEOcoinBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getLEOcoinBalance() const
{
    return wallet->GetLEOcoinBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newLEOcoinBal = getLEOcoinBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedLEOcoinBal != newLEOcoinBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newLEOcoinBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("LEOcoin is not connected!"));
    ...
    {
        nBalance = wallet->GetLEOcoinBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsLEOcoinBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_LEO_TO_LEO
                || rcp.txnTypeInd == TXT_ANON_TO_LEO)
            {
                // -- out LEO
                std::string sError;
...
{
    return BackupWallet(*wallet, filename.toStdString().data());
}
