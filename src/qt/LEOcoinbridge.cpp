#include "LEOcoinbridge.h"

#include "LEOcoingui.h"
#include "guiutil.h"
...
#include "LEOcoinbridge.moc"

LEOcoinBridge::LEOcoinBridge(LEOcoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void LEOcoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void LEOcoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void LEOcoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void LEOcoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void LEOcoinBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void LEOcoinBridge::populateOptions()
{
...
// Transactions
bool LEOcoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void LEOcoinBridge::clearRecipients()
{
    recipients.clear();
}

bool LEOcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 LEO, 1 LEOcoin
        switch(rcp.txnTypeInd)
        {
            case TXT_LEO_TO_LEO:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_LEO_TO_ANON:
                formatted.append(tr("<b>%1</b> to LEO %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> LEO, ring size %2 to LEO %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_LEO:
                formatted.append(tr("<b>%1</b> LEO, ring size %2 to LEO %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsLEOcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your LEOcoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LEO, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void LEOcoinBridge::openCoinControl()
{
...
void LEOcoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void LEOcoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap LEOcoinBridge::listAnonOutputs()
{
...
void LEOcoinBridge::populateTransactionTable()
{
...
void LEOcoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void LEOcoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString LEOcoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void LEOcoinBridge::populateAddressTable()
{
...
void LEOcoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void LEOcoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString LEOcoinBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString LEOcoinBridge::lastAddressError()
{
    QString sError;
...
QString LEOcoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void LEOcoinBridge::updateAddressLabel(QString address, QString label)
{
...
// Messages
void LEOcoinBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void LEOcoinBridge::appendMessage(int row)
{
...
void LEOcoinBridge::populateMessageTable()
{
...
void LEOcoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool LEOcoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool LEOcoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString LEOcoinBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString LEOcoinBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool LEOcoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool LEOcoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString LEOcoinBridge::createGroupChat(QString label)
{
...
QString LEOcoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList LEOcoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void LEOcoinBridge::connectSignals()
{
...
QString LEOcoinBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap LEOcoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap LEOcoinBridge::listLatestBlocks()
{
...
QVariantMap LEOcoinBridge::findBlock(QString searchID)
{
...
QVariantMap LEOcoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap LEOcoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap LEOcoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap LEOcoinBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap LEOcoinBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap LEOcoinBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap LEOcoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap LEOcoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap LEOcoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap LEOcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with LEOcoin prefix.");
            return result;
        }
...
QVariantMap LEOcoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap LEOcoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap LEOcoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
