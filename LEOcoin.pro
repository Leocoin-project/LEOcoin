...
build_macosx64 {
    QMAKE_TARGET_BUNDLE_PREFIX = co.LEOcoin
    BOOST_LIB_SUFFIX=-mt
...
contains(LEO_NEED_QT_PLUGINS, 1) {
    DEFINES += LEO_NEED_QT_PLUGINS
...
SOURCES += \
    ...
    src/qt/LEOcoingui.cpp \
    src/qt/LEOcoin.cpp \
    src/qt/LEOcoinbridge.cpp
...
# also add new translations to LEOcoin.qrc under translations/
TRANSLATIONS = $$files(src/qt/locale/LEOcoin*.ts)
...
LIBS += -lboost_system$$BOOST_LIB_SUFFIX -lboost_filesystem$$BOOST_LIB_SUFFIX -lboost_program_options$$BOOST_LIB_SUFFIX -lboost_thread$$BOOST_THREAD_LIB_SUFFIX -lboost_locale$$BOOST_LIB_SUFFIX
